...
var exmo=exmo||{},Twitch=Twitch||{};exmo.model=exmo.model||{},exmo.model.User=function(){function prop(name){return 2==arguments.length&&(this[name]=arguments[1]),this[name]}function serialize(){return{username:prop("username"),email:prop("email"),displayName:prop("displayName")}}function parse(data){return prop("username",data.name),prop("email",data.email),prop("displayName",data.display_name),this}return{prop:prop,parse:parse,serialize:serialize}},!function($){$(document).on("ready",function(){$(".twitch-connect").hide(),Twitch.init({clientId:exmo.twitch.client.id},function(error,status){$(".twitch-connect").on("click",function(){Twitch.login({scope:["user_read"]})}),status.authenticated||$(".twitch-connect").show()})})}(jQuery);